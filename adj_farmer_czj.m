...
[bid_orderBook,ask_orderBook,y_bid,y_ask]=InitBook_czj(y_bid,y_ask);            %初始化订单簿，生成各3个报价
% [E1,index1]=sortrows(bid_orderBook,-1);                                     %bid从高到低关联排序
[E1,index1]=sortrows(bid_orderBook(:,2),-1)
% [E2,index2]=sortrows(ask_orderBook,1);                                      %ask从低到高关联排序
[E2,index2]=sortrows(ask_orderBook(:,2),1);    
%订单簿最优报价
best_bid(1,1)=E1(1,1);
best_ask(1,1)=E2(1,1);

bid_delta_0=best_ask(1,1)*ones(3,1)-bid_orderBook(index1,2);                %0时刻买方订单i的delta值，行表示订单编号，t表示时间
ask_delta_0=ask_orderBook(index2,2)-best_bid(1,1)*ones(3,1);                %0时刻卖方订单i的delta值，行表示订单编号，t表示时间
...
% % % % % % % % % % % % % 订单类型判断及限价、市场订单的产生、交易% % % % % % % % % % % % % 
 ...
 if order_sign(1,i-1)>0                                                     %买入：+1
        if price_distance<spread(i-1,1)                                           %买入限价订单,修改spread->spread(i,1)
            bid_num=bid_num+1;                                              %买入限价订单号加1
            bid_orderBook=[ bid_orderBook;bid_num  best_bid(i-1,1)+price_distance];
            y_bid(i,bid_num)=1;                                             %产生初始y值
            
        else                                                                %产生买入市场订单
             if size(ask_orderBook,1)>2                                             %要求订单簿中的卖出限价订单簿数量大于2，否则为无效市场订单
                 ask_orderBook(index2(1),:)=[];
             end
        end
 else                                                                       %卖出：-1
     if price_distance<spread(i-1,1)                                              %卖出限价订单
         ask_num=ask_num+1;                                                 %卖出限价订单号加1
         ask_orderBook=[ask_orderBook;ask_num  best_ask(i-1,1)-price_distance];
         y_ask(i,ask_num)=1;
     else                                                                   %产生卖出市场订单
         if size(bid_orderBook,1)>2                                                 %要求订单簿中的买入限价订单簿数量大于2，否则为无效市场订单
...
y_bid(i,:)=(best_ask(i-1,1)*ones(BidLen,1)-bid_orderBook(:,2))./bid_delta_0;  %买单时刻t的y值
